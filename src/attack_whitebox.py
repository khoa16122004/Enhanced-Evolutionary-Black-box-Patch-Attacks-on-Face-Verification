...
def ii_fo(img1, img2, facial_encode_model, steps=80, alpha=0.01, epsilon=0.01):
    ...
    for step in range(steps):
        ...
        loss = -loss  # maximize similarity -> minimize negative
        loss.backward()
        with torch.no_grad():
            delta.data = torch.clamp(delta - alpha * torch.sign(delta.grad), -epsilon, epsilon)
        delta.grad.zero_()
        print(f"Step {step}: loss = {-loss.item()}")  # inverting back for readability
